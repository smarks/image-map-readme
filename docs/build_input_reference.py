...
def render_workflow() -> str:
    ...
    stages = [
        ...
        (
            "Go live (blue-green)",
            "<code>roots.origamisoftware.com</code> deploys onto the server with a "
            "blue-green swap — the new build is brought up beside the running one and "
            "switched in, so there's no downtime. nginx then serves the one static "
            "gated file at <strong>roots.origamisoftware.com/me</strong>. The "
            "visitor's answer decrypts the poster in-browser; the page only ever "
            "ships ciphertext.",
        ),
    ]
    blocks = []
    ...
    return f"""        <section class="screen workflow">
            ...
            <div class="workflow-note">
                <strong>This all runs as a GitHub Workflow.</strong> You commit a file
                to the private repo and wait — there are no manual build or deploy
                steps. The change then shows up live, and the server brings the new
                version up with a <strong>blue-green deployment</strong> (the new build
                is staged beside the running one and swapped in, so there's no downtime).
            </div>
            <ol class="flow">
{flow_html}
            </ol>
